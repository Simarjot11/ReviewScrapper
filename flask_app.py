...
'''
1.default when we click on any url on google it opens is GET method hence this lines fail and it enters to else statement
2.When we click on search on the html page we call index function with POST method
'''


@app.route('/',methods=['POST','GET']) # route with allowed methods as POST and GET
def index():
    if request.method == 'POST': #default when we click on any url on google it opens is GET method hence this lines fail and it enters to else statement
        searchString = request.form['content'].replace(" ","") # obtaining the search string entered in the form
        try:
            ''' ESTABLISHING  RELATION WITH DATABASE'''
            ...
            if reviews.count() > 0: # if there is a collection with searched keyword and it has records in it
                ...
                flipkart_html = bs(flipkartPage, "html.parser") # parsing the webpage as HTML format
                ...
                box = bigboxes[0] #  taking the first item (for demo)
